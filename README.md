...
```bash
# 备份
docker-compose exec db pg_dump -U rss_post rss_post | gzip > backup_$(date +%Y%m%d).sql.gz

# 恢复
gunzip -c backup_20240115.sql.gz | docker-compose exec -T db psql -U rss_post rss_post
```
